...
export const name = 'heap-snapshot-viewer.error-invalid-edge-target'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Workspace }) => {
  ...
  const uri = `${tmpDir}/invalid-edge-target.heapsnapshot`
  await FileSystem.writeFile(uri, JSON.stringify(snapshot))
  await Workspace.setPath(tmpDir)
  await Main.openUri(uri)

  const errorMessage = Locator('.HeapSnapshotErrorMessage')
  await expect(errorMessage).toHaveText('Edge 0 points to an invalid node index.')
}
